...
- `ps2write` passes plain images through at their source resolution, but a page it cannot keep as vectors (transparency, which every Canva export uses) is rasterised whole at the device resolution. Always pass `-r` from the PPD's `*DefaultResolution`: on its 720 dpi default such a page costs hundreds of MB and can outrun the conversion timeout. `fixtures/` has no such PDF; `test/ppd.test.ts` builds one.
- A page device `Install` hook runs several times per page (12 for a 5-page job), so it cannot count pages or carry per-page state; give it constants. Page boxes come from `runpdfbegin`/`pdfgetpage` under `-dNODISPLAY`, which reads the whole document in well under a second.
